 ...
 *
 *  https://bitbucket.org/kwi/urllinker/src (using htmles instead of htmlspecialchars)
 */
...
function htmlEscapeAndLinkUrls($text)
{
    ...
    while (preg_match($rexUrlLinker, $text, $match, PREG_OFFSET_CAPTURE, $position))
    {
        ...
        $html .= htmles(substr($text, $position, $urlPosition - $position));
        ...
        if (preg_match('{^\.[0-9]{1,3}$}', $tld) || isset($validTlds[$tld]))
        {
            ...
            $linkHtml = '<a href="' . htmles($completeUrl) . '">'
                . htmles($linkText)
                . '</a>';
    ...
    $html .= htmles(substr($text, $position));
    return $html;
}
